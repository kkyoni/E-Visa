...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-12 color_title">
        <h2><i class="fa fa-question-circle"></i> Pre Payment Question Management</h2>
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <a href="{{ route('admin.dashboard') }}">Home</a>
            </li>
            <li class="breadcrumb-item active">
                <strong>Pre Payment Question Management Table</strong>
            </li>
        </ol>
    </div>
</div>
			...
			<div class="ibox ">
				<div class="ibox-content">
                    ...
                    <div class="col-md-12 text-right">
                        ...
                        @endphp
                        @if($checkPermission)
                        <a class="btn btn-sm btn-primary  pull-right mb-3 op-btn" href="{{route('admin.pre.create')}}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title pull-left" id="exampleModalLabel1">Pre Payment Question Detail</h5>
...
<script type="text/javascript">
	$(document).on("click",".changeStatusRecord",function(e){
		...
		swal({
			...
			if (isConfirm) {
				$.ajax({
					url:"{{ route('admin.pre.change_status','replaceid') }}",
					type: 'post',
					data: {"_method": 'post',
                  'id':id,
                  "_token": "{{ csrf_token() }}"
    ...
    $('#country_id').change(function () {
        ...
        if(country_id){
            window.location.replace("{{route('admin.pre.country_index',[''])}}"+"/"+country_id);
        }else{
            window.location.replace("{{route('admin.pre.index')}}");
        }
    })
    $('#visa_type_id').change(function () {
        var  visa_type_id=$(this).val();
        if(visa_type_id){
            window.location.replace("{{route('admin.pre.visatype_index',[''])}}"+"/"+visa_type_id);
        }else{
            window.location.replace("{{route('admin.pre.index')}}");
        }
    })
