...
class PostController extends Controller{
    ...
    */
    public function __construct(){
        ...
        $this->pageLayout = 'admin.pages.post.';
    ...
    public function index(Builder $builder, Request $request){
        ...
        $postpayment = PrePostPayment::where('payment_status','post')->groupBy('u_id')->orderBy('id','desc');
        if (request()->ajax()) {
            return DataTables::of($postpayment->get())
            ->addIndexColumn()
            ->editColumn('status', function (PrePostPayment $postpayment) {
                if ($postpayment->status == "active") {
            ...
            ->editColumn('action', function (PrePostPayment $postpayment)  use($permission_data) {
                $action = '';
                if($permission_data['hasUpdatePermission']){
                    $action .='<a class="btn btn-info btn-sm ml-2 mr-2 viewpre" data-id ="'.$postpayment->u_id.'" href="javascript:void(0)" title="View"><i class="fa fa-eye"></i></a>';
                    $action .='<a href='.route('admin.post.edit',[$postpayment->u_id]).' class="btn btn-warning btn-sm" title="Edit"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;';
                }
                if($permission_data['hasRemovePermission']){
                    $action .='<a class="btn btn-danger btn-sm m-l-1 deletepre" data-id ="'.$postpayment->u_id.'" href="javascript:void(0)" title="Delete"><i class="fa fa-trash"></i></a>';
                }
                if($permission_data['hasUpdatePermission']){
                    if($postpayment->status == "active"){
                        $action .= '<a href="javascript:void(0)" data-value="1"   data-toggle="tooltip" title="Active" class="btn btn-sm btn-dark ml-2 mr-2 changeStatusRecord" data-id="'.$postpayment->u_id.'" title="Active"><i class="fa fa-unlock"></i></a>';
                    }else{
                        $action .= '<a href="javascript:void(0)" data-value="0"  data-toggle="tooltip" title="Block" class="btn btn-sm btn-dark ml-2 mr-2 changeStatusRecord" data-id="'.$postpayment->u_id.'" title="Block"><i class="fa fa-lock"></i></a>';
                    }
                }
                return $action;
            })
            ...
            ->editColumn('answer', function (PrePostPayment $prepayment) {
    ...
    /* -----------------------------------------------------------------------------------------
    @Description: Function for store Post data
    @input: Post
    @Output: store Post data
    -------------------------------------------------------------------------------------------- */
    public function store(Request $request){
        ...
        try{
            ...
            if($request->answer_type === 'text' || $request->answer_type === 'datepicker'){
                ...
                $prepostpayment['payment_status'] =  "post";
                ...
                if(isset($request->sub_question)){
                    AdminSubQuestion::create([
                        'subque_id'             =>  $prepost->id,
                        'sub_question'          =>  $request->sub_question,
                        'sub_answer_type'       =>  $request->sub_answer_type,
                        'sub_add_droup'         =>  $request->sub_add_droup,
                        'sub_note'              =>  $request->sub_note,
                        'sub_tooltip'           =>  $request->sub_tooltip,
                        'sub_proceed'           =>  $sub_proceed,
                    ]);
                }
            }
            Notify::success('Post Payment Created Successfully.');
    ...
    /* -----------------------------------------------------------------------------------------
    @Description: Function for update Post data
    @input: Post
    @Output: Update Post data
    -------------------------------------------------------------------------------------------- */
    public function update(Request $request, $id){
        ...
        try{
            if($request->answer_type === 'text' || $request->answer_type === 'datepicker'){
                ...
                if(isset($request->sub_question)){
                    $prepost = PrePostPayment::where('u_id', $id)->first();
                    $checksubque = AdminSubQuestion::where('subque_id', $prepost->id)->first();
                    if(!empty($checksubque)){
                        AdminSubQuestion::where('subque_id', $prepost->id)->update([
                            'sub_question'          =>  $request->sub_question,
                            'sub_answer_type'       =>  $request->sub_answer_type,
                            'sub_add_droup'         =>  $request->sub_add_droup,
                            'sub_note'              =>  $request->sub_note,
            ...
            Notify::success('Post Question Updated Successfully.');
            return redirect()->route('admin.post.index');
        }catch(\Exception $e){
            return back()->with([
                'alert-type'    => 'danger',
    ...
    /* -----------------------------------------------------------------------------------------
    @Description: Function for Post Status
    @input: Post
    @Output: Status Post Change 
    -------------------------------------------------------------------------------------------- */
    ...
    public function delete(PrePostPayment $PrePostPayment, $id){
        try{
            ...
            return response()->json([
                'status'    => 'success',
                'title'     => 'Success!!',
                'message'   => 'Post Question Deleted successfully.'
            ]);
        }catch(\Exception $e){
            return back()->with([
                'alert-type'    => 'danger',
    ...
    public function country_index(Builder $builder, Request $request, $id){
        ...
        if (request()->ajax()) {
            ...
            ->editColumn('action', function (PrePostPayment $postpayment) {
                $action = '';
                $action .='<a class="btn btn-info btn-sm ml-2 mr-2 viewpre" data-id ="'.$postpayment->u_id.'" href="javascript:void(0)"><i class="fa fa-eye"></i></a>';
                $action .='<a href='.route('admin.post.edit',[$postpayment->u_id]).' class="btn btn-warning btn-sm"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;';
                $action .='<a class="btn btn-sm btn-danger m-l-1 deletepre" data-id ="'.$postpayment->u_id.'" href="javascript:void(0)"><i class="fa fa-trash"></i></a>';
                if($postpayment->status == "active"){
                    $action .= '<a href="javascript:void(0)" data-value="1"   data-toggle="tooltip" title="Active" class="btn btn-sm btn-dark ml-2 mr-2 changeStatusRecord" data-id="'.$postpayment->u_id.'"><i class="fa fa-unlock"></i></a>';
                }else{
                    $action .= '<a href="javascript:void(0)" data-value="0"  data-toggle="tooltip" title="Block" class="btn btn-sm btn-dark ml-2 mr-2 changeStatusRecord" data-id="'.$postpayment->u_id.'"><i class="fa fa-lock" ></i></a>';
                }
                return $action;
            })
            ->editColumn('u_id', function (PrePostPayment $prepayment) {
    ...
    /* -----------------------------------------------------------------------------------------
    @Description: Function for index
    @input: Post For Visa type Index
    @Output: Details data view
    -------------------------------------------------------------------------------------------- */
